...
import { IceCreamBowl, ChevronDown, ChevronLeft, ChevronRight, Search, ShoppingBag, SlidersHorizontal, UserRound } from 'lucide-react'
...
export default function ProductsPage() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`상품 목록을 불러오지 못했습니다. (${response.status})`)
      ...
      setProducts([])
      setTotalPages(1)
      setError(requestError instanceof Error ? requestError.message : '상품 목록을 불러오지 못했습니다.')
    } finally {
      setLoading(false)
    }
  ...
  return (
    <main className="min-h-screen bg-[#f5f7fa] text-[#0a1f3d]">
      <header className="border-b border-[#dce5ef] bg-white/90 px-6 py-5 backdrop-blur sm:px-10 lg:px-16">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div className="flex items-center gap-5 text-[#52657c]">
            <button className="hidden items-center gap-1 text-sm font-medium sm:flex" onClick={() => router.push(userName ? '/mypage' : '/login')}><UserRound className="size-4" aria-hidden="true" />{userName || '로그인'}</button>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-14 pt-14 sm:px-10 lg:px-16 lg:pt-20">
        ...
        <button type="button" onClick={() => setFilterOpen((open) => !open)} className="mt-8 flex w-full items-center justify-center gap-2 rounded-xl border border-[#d8e1ec] bg-white px-4 py-3 text-sm font-bold text-[#0a1f3d] shadow-sm md:hidden"><SlidersHorizontal aria-hidden="true" />필터 {filterOpen ? '닫기' : '열기'}</button>

        <div className="mt-8 grid items-start gap-8 md:grid-cols-[minmax(190px,24%)_1fr] lg:gap-10">
          <aside className={`${filterOpen ? 'block' : 'hidden'} rounded-2xl border border-[#d8e1ec] bg-white p-5 shadow-sm md:sticky md:top-6 md:block`} aria-label="상품 필터">
            <div className="flex flex-col gap-3"><h2 className="text-base font-bold">카테고리</h2>{categories.map((item) => <label key={item} className={`flex cursor-pointer items-center gap-3 rounded-lg px-2 py-2 text-sm transition ${category === item ? 'bg-[#eaf3ff] font-bold text-[#1265d8]' : 'text-[#52657c] hover:bg-[#f5f7fa]'}`}><input type="radio" name="category" value={item} checked={category === item} onChange={() => chooseCategory(item)} className="accent-[#1265d8]" />{categoryLabels[item]}</label>)}</div>
            <div className="my-6 border-t border-[#e8eef5]" />
            <form onSubmit={(event) => { event.preventDefault(); applyPriceFilter() }} className="flex flex-col gap-3" aria-label="가격 범위 필터"><h2 className="text-base font-bold">가격 범위</h2><label className="flex flex-col gap-2 text-xs font-semibold text-[#52657c]">최소가<input type="number" min="0" value={minPrice} onChange={(event) => setMinPrice(event.target.value)} placeholder="최소 가격" className="rounded-xl border border-[#d8e1ec] px-3 py-2.5 text-sm text-[#0a1f3d] outline-none transition focus:border-[#159c98] focus:ring-2 focus:ring-[#7fdbda]/40" /></label><label className="flex flex-col gap-2 text-xs font-semibold text-[#52657c]">최대가<input type="number" min="0" value={maxPrice} onChange={(event) => setMaxPrice(event.target.value)} placeholder="최대 가격" className="rounded-xl border border-[#d8e1ec] px-3 py-2.5 text-sm text-[#0a1f3d] outline-none transition focus:border-[#159c98] focus:ring-2 focus:ring-[#7fdbda]/40" /></label><button type="submit" className="mt-1 rounded-xl bg-[#0a1f3d] px-5 py-2.5 text-sm font-bold text-white transition hover:bg-[#1265d8]">적용</button></form>
          </aside>
          <section className="min-w-0">
        {loading ? <div className="mt-8 grid gap-x-5 gap-y-12 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{Array.from({ length: 8 }, (_, index) => <ProductSkeleton key={index} />)}</div> : error ? <div role="alert" className="mt-8 rounded-[24px] border border-red-200 bg-red-50 px-6 py-16 text-center"><h2 className="text-lg font-bold text-red-700">상품을 불러오지 못했습니다.</h2><p className="mt-3 text-sm text-red-600">{error}</p><p className="mt-2 text-xs text-red-500">잠시 후 다시 시도해주세요.</p></div> : products.length === 0 ? <div className="mt-8 rounded-[24px] border border-dashed border-[#cbd8e7] bg-white px-6 py-24 text-center"><ShoppingBag className="mx-auto size-10 text-[#9aacc0]" /><h2 className="mt-5 text-lg font-bold">상품이 아직 없어요</h2><p className="mt-2 text-sm text-[#64748b]">다른 카테고리를 선택해보세요.</p></div> : <div className="mt-8 grid gap-x-5 gap-y-12 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{products.map((product, index) => { const unavailable = product.status === '품절' || product.status === '판매 중지'; return <button key={product.id} onClick={() => router.push(`/products/${product.id}`)} className={`group text-left ${unavailable ? 'opacity-55' : ''}`} aria-label={`${product.name} 상세 보기`}><div className="relative flex aspect-[4/3] items-end overflow-hidden rounded-[22px] bg-gradient-to-br from-[#dceeff] via-[#eef8fb] to-[#c8ddf2] p-5 transition duration-300 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-[#1265d8]/10"><span className="text-6xl font-bold tracking-[-0.12em] text-[#1265d8]/15">{String(index + 1).padStart(2, '0')}</span><span className={`absolute right-4 top-4 rounded-full px-3 py-1 text-[11px] font-bold ${unavailable ? 'bg-slate-500/80 text-white' : 'bg-white/85 text-[#1265d8]'}`}>{product.status}</span></div><p className="mt-4 text-xs font-bold tracking-[0.16em] text-[#159c98]">{product.category}</p><h2 className="mt-2 text-lg font-bold tracking-[-0.03em]">{product.name}</h2><p className="mt-2 text-sm font-semibold text-[#52657c]">{formatPrice(product.price)}</p></button> })}</div>}

        {totalPages > 1 && <nav className="mt-16 flex flex-wrap items-center justify-center gap-2" aria-label="상품 페이지 탐색"><button disabled={page === 0} onClick={() => setPage(0)} className="rounded-full border border-[#d8e1ec] bg-white px-3 py-2 text-xs font-bold text-[#52657c] transition hover:border-[#1265d8] hover:text-[#1265d8] disabled:cursor-not-allowed disabled:opacity-35">처음</button><button disabled={page === 0} onClick={() => setPage((current) => current - 1)} className="grid size-9 place-items-center rounded-full border border-[#d8e1ec] bg-white disabled:opacity-35" aria-label="이전 페이지"><ChevronLeft /></button>{getPaginationItems(page, totalPages).map((item, index) => item === 'ellipsis' ? <span key={`ellipsis-${index}`} className="px-1 text-sm text-[#94a3b8]" aria-hidden="true">...</span> : <button key={item} onClick={() => setPage(item)} aria-current={page === item ? 'page' : undefined} className={`grid size-9 place-items-center rounded-full text-sm font-semibold ${page === item ? 'bg-[#1265d8] text-white' : 'text-[#64748b] hover:bg-white'}`}>{item + 1}</button>)}<button disabled={page >= totalPages - 1} onClick={() => setPage((current) => current + 1)} className="grid size-9 place-items-center rounded-full border border-[#d8e1ec] bg-white disabled:opacity-35" aria-label="다음 페이지"><ChevronRight /></button><button disabled={page >= totalPages - 1} onClick={() => setPage(totalPages - 1)} className="rounded-full border border-[#d8e1ec] bg-white px-3 py-2 text-xs font-bold text-[#52657c] transition hover:border-[#1265d8] hover:text-[#1265d8] disabled:cursor-not-allowed disabled:opacity-35">끝</button></nav>}
          </section>
        </div>
      </section>
    </main>
